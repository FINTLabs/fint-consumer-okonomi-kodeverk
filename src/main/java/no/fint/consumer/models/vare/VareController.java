...
@CrossOrigin
@RestController
@RequestMapping(name = "Vare", value = RestEndpoints.VARE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class VareController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private VareCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private VareLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Vare cache is disabled.");
        }
    ...
    public VareResources getVare(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getVareByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Vare cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_VARE, client);
    ...
    @PostMapping("/$query")
    public VareResources getVareByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getVare(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private VareResources getVareByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_VARE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new VareResources();
    
        ArrayList<VareResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<VareResource>>() {});
    ...
    public VareResource getVareBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_VARE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<VareResource> vare = cacheService.getVareBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return vare.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            VareResource vare = objectMapper.convertValue(response.getData().get(0), VareResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(vare);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, VareResource.class);
    }

    @PostMapping
    public ResponseEntity postVare(
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody VareResource body,
            @RequestParam(name = "validate", required = false) boolean validate
    ) {
        log.debug("postVare, Validate: {}, OrgId: {}, Client: {}", validate, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.UPDATE_VARE, client);
    ...
    public ResponseEntity putVareBySystemId(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody VareResource body
    ) {
        log.debug("putVareBySystemId {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.UPDATE_VARE, client);
        event.setQuery("systemid/" + id);
