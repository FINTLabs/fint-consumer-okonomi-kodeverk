...
@Slf4j
@Api(tags = {"Merverdiavgift"})
@CrossOrigin
@RestController
@RequestMapping(name = "Merverdiavgift", value = RestEndpoints.MERVERDIAVGIFT, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class MerverdiavgiftController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private MerverdiavgiftCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private MerverdiavgiftLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Merverdiavgift cache is disabled.");
        }
    ...
    @GetMapping
    public MerverdiavgiftResources getMerverdiavgift(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getMerverdiavgiftByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Merverdiavgift cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_MERVERDIAVGIFT, client);
        ...
        Stream<MerverdiavgiftResource> resources;
    ...
    @PostMapping("/$query")
    public MerverdiavgiftResources getMerverdiavgiftByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getMerverdiavgift(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private MerverdiavgiftResources getMerverdiavgiftByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_MERVERDIAVGIFT, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new MerverdiavgiftResources();
    
        ArrayList<MerverdiavgiftResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<MerverdiavgiftResource>>() {});
    ...
    public MerverdiavgiftResource getMerverdiavgiftBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_MERVERDIAVGIFT, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<MerverdiavgiftResource> merverdiavgift = cacheService.getMerverdiavgiftBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return merverdiavgift.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            MerverdiavgiftResource merverdiavgift = objectMapper.convertValue(response.getData().get(0), MerverdiavgiftResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(merverdiavgift);
        }    
    }
